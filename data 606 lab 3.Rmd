...
Answer 1: The streak is Hit until Miss. So if Kobe shots 5 Hits back to back and misses, the streak would be HHHHHM. Consecutive hits. Streak length of 1 would mean Hit and then Miss. Streak length 0 would be if Kobe misses back to back. So if Koby missed 5 shots in a row, there are 5 streaks, each streak has 0 length.
...
Answer: The distribution of Kobe’s streak lengths are unimodal and right skewed. His typical streak length was 1. His longest streak of baskets were 4.
...
### 3. In your simulation of flipping the unfair coin 100 times, how many flips came up heads? Include the code for sampling the unfair coin in your response.
```{r}
sim_unfair_coin
```
```{r}
...
#### answer: 22 times heads and 78 times tails came up
...
answer: The distribution of streak lengths is unimodal and right skewed. Typical streak length is 1. The longest streak is 8.
...
answer: With the same probability of 0.45, i would expect it to be somewhat similar to the simulation above. Since they are independent and we are looking at the same amount of shots (133), i would expect the result to be similar.
...
answer: The distribution is similar, (unimodal with right skewed) and the typical streak length is similar. However, the simulated shooter has better longest streak (8), compare to Kobe’s longest streak length. I don't see any evidence that the hot hand model fits Kobe’s shooting pattern. The event of shots are independent from each other.
